...
public class DevolverSerie extends JFrame {
	...
	public void crear(int id, String username) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DevolverSerie frame = new DevolverSerie(id, username);
	 ...
	 * Create the frame.
	 */
	public DevolverSerie(int id, String username) {
		setTitle("Devolver Series Rentadas");
		this.idUsuario = id;
		this.user = username;
		...
		JLabel lblNoCuentaCon = new JLabel("No cuenta con series rentadas!");
		...
		try {
			...
			ResultSet rs=stmt.executeQuery("SELECT "
											+ "s.idSerie AS id, "
											+ "s.nombreSerie AS \"Nombre de Serie\", " 
											+ "s.temporadas AS Temporadas, " 
											+ "s.productora AS Productora, "
											...
											+ "CASE "
												...
												+ "END "
											+ "AS Devolver "
										+ "FROM seriesRentadas sr "
										+ "INNER JOIN usuarios u ON u.idUsuario = sr.idUsuario "
										+ "INNER JOIN series s	 ON s.idSerie = sr.idSerie "
										+ "WHERE sr.idUsuario = " + idUsuario + ";");
		...
		table.addMouseListener(new java.awt.event.MouseAdapter() {
		    ...
		    public void mouseClicked(java.awt.event.MouseEvent evt) {
		        ...
		        Object idSerie = table.getValueAt(row,0);
		        if (rentar.equals("Devolver") && col == 5) {
		        	...
		        	if(dialogResult == JOptionPane.YES_OPTION){
		        		try {
		        			...
		        			stmt.execute("DELETE FROM seriesRentadas "
		        						+ "WHERE idUsuario = "+ idUsuario + " "
		        							+ "AND idSerie = " + idSerie + ";");
		        				stmt.close();
		        				con.close();
		        				DevolverSerie ds = new DevolverSerie(idUsuario, user);
		        				ds.crear(idUsuario, user);
		    					dispose();
		        		}
		        			catch(Exception e)
		        		{
		...
		gl_panel.setHorizontalGroup(
			gl_panel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel.createSequentialGroup()
					...
					.addComponent(lblNoCuentaCon, GroupLayout.DEFAULT_SIZE, 366, Short.MAX_VALUE)
					.addGap(80))
		);
